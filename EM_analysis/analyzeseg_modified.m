classdef analyzeseg_modified < handle
% This is a slighly modified version of the class analyzeseg()     
% The original class, created by Hongh-Hsi Lee, can be downloaded from:
% https://github.com/NYU-DiffusionMRI/RaW-seg
% this class needs the files: TR_high.mat, TR_low.mat, TR_medium.mat 
    ...
    methods (Access = public)
        function this = analyzeseg_modified(varargin)
...
%   map, and u and R are 3-element vectors.
        ...
        function fb = fiberdiameterRot(this,fiber,myelin,sigma,diametermax,vox)
...
%             nslice = size(cm,1);
            ...
            incircle      = zeros(nslice,1);
            outerdiameter = zeros(nslice,1);
            for i = zlist%1:nslice
                fiberi = squeeze(fiber(:,:,i));
%                 fiberi = this.bwalign(fiber,cm(i,:)./vox,tangent(i,:),[fov fov 1]);
                if nnz(fiberi)==0, continue; end
                diameter(i) = sqrt(sum(fiberi(:))/pi)*2;
                stats = regionprops(fiberi,'minoraxislength','majoraxislength','area');
                ...
                D = bwdist(~fiberi);
                incircle(i) = max(D(:))/10*2;
                myelini = squeeze(myelin(:,:,i) | fiber(:,:,i));
%                 myelini = this.bwalign(logical(myelin+fiber),cm(i,:)./vox,tangent(i,:),[fov fov 1]);
                outerdiameter(i) = sqrt(sum(myelini(:))/pi)*2;
            end
            fb.diameter      = diameter*vox(1);
    ...
    methods (Static)
        function cm = centermassz(fiber,vox)
            [nx,ny,~] = size(fiber);
            zlist = find(sum(sum(fiber,1),2));
        ...
        function cm = centermass(fiber,vox)
            ...
            [~,ins]    = max([nx ny nz]);    
            if(ins == 1)
            xlist = find(sum(sum(fiber,3),2));
            cm    = zeros(numel(xlist),3);
            for i = 1:numel(xlist)
                fiberi  = squeeze(fiber(xlist(i),:,:));
                [I,J]   = ind2sub([ny,nz],find(fiberi));
                cm(i,:) = [xlist(i)*vox(1) mean(I)*vox(2) mean(J)*vox(3)];
            end % i
            elseif(ins == 2)
            ylist = find(sum(sum(fiber,3),1));   
            cm    = zeros(numel(ylist),3);
            for i = 1:numel(ylist)
                fiberi  = squeeze(fiber(:,ylist(i),:));
                [I,J]   = ind2sub([nx,nz],find(fiberi));
                cm(i,:) = [mean(I)*vox(1) ylist(i)*vox(2) mean(J)*vox(3)];
            end % i
            elseif(ins == 3)
            zlist = find(sum(sum(fiber,1),2));  
            cm    = zeros(numel(zlist),3);
            for i = 1:numel(zlist)
                fiberi = squeeze(fiber(:,:,zlist(i)));
                [I,J] = ind2sub([nx,ny],find(fiberi));
                cm(i,:) = [mean(I)*vox(1),mean(J)*vox(2),zlist(i)*vox(3)];
            end % i
            else
            fprintf('error centermass')    
            end % if - ns
        end
        ...
        function [fod,points,fodori,solang] = fodsphere(tangent,varargin)
...
% author: Hong-Hsi Lee, 2017
            ...
            str = which('analyzeseg_modified.m');
            [root,~,~] = fileparts(str);
            ...
            vi = nearestNeighbor(TR,tangent); % point closest to each tg (returns IDs)
            fod = zeros(size(K,1),1);
            for i = 1:size(tangent,1)
                ti = vertexAttachments(TR,vi(i)); % trangles that have vi(i) as one vertex (returns IDs)
                N = neighbors(TR,ti{:}.'); % neighbors (triangles) of the previous triangles (returns IDs)
                N = unique([N(:); ti{:}.']); % remove repeated (returns IDs)
                B = cartesianToBarycentric(TR,N,repmat(tangent(i,:),[numel(N),1])); % find corrdinates of tg with respect to all triangles
                I = find( ( B(:,1)>=0 ) .* ( B(:,2)>=0 ) .* ( B(:,3)>=0 ) ); % find the triangle that really contains tg (all positive )
                if isempty(I)
                    continue
                end
                I = N(I(1));
...
% 3x faster but it does not give the same (or similar) results (pl. lambda, C)
% tic
% ndiv = 20000;
% fod2 = zeros(size(pts,1),1);
% nn   = ceil(size(tangent,1)/ndiv);
% k    = 0;
% for i = 1:nn
% ki = k + 1;
% ke = min([k+ndiv size(tangent,1)]);
% pp = pts*tangent(ki:ke,:)';   
% [~,ppi]   = max(pp);
% fod2(ppi) = fod2(ppi) + 1;
% k         = i*ndiv;
% end
...
% author: Hong-Hsi Lee, 2018
        ...
        function [pl,plm] = rotinv(fod,nord,flag)
